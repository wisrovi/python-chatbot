...
def mensaje_nocomando(update, context):
    ...
    answer, tag = processor.chatbot_response(question_user) # envio el mensaje al chatbot y obtengo la respuesta
    ...
    print(f"[USER]: {str(cid)} [QUESTION]: {question_user} [ANSWER]: {answer} [TAG]: {tag}")
